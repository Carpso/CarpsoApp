...
export default function ParkingLotManager() {
  ...
  useEffect(() => {
   ...
   // Effect to show voice assistant errors from the hook
   useEffect(() => {
   ...
   // eslint-disable-next-line react-hooks/exhaustive-deps
   }, [isClient, voiceError]); // Depend on isClient and the error status from the hook result
  ...
  useEffect(() => {
   ...
   // eslint-disable-next-line react-hooks/exhaustive-deps
   }, [isLoadingLocations, isLoadingBookmarks, isAuthenticated, fetchRecommendations]); // Added fetchRecommendations
 ...
 const getVoiceButtonIcon = () => {
       // Return placeholder or static icon before client-side hydration or if not supported/offline
       if (!isClient || !isVoiceSupported || !isOnline) {
            return <MicOff key="disabled-mic" className="h-5 w-5 text-muted-foreground opacity-50" />;
       }
       // Logic runs only on the client when supported and online
  ...
  const handleVoiceButtonClick = () => {
      // Use functions from the hook result
       if (!isClient || !isVoiceSupported || !isOnline) return;
       // Button now just toggles continuous listening on/off
       if (voiceAssistantState === 'listening' || voiceAssistantState === 'activated') {
           stopListening();
       } else {
           startListening();
       }
   ...
   const getVoiceButtonTooltip = () => {
       ...
       if (!isClient) return "Loading voice assistant...";
        ...
        switch (voiceAssistantState) {
             ...
             case 'error': return `Error: ${voiceError || 'Unknown'}`; // Use error from hook result
             case 'idle':
 ...
 return (
   <div className="container py-8 px-4 md:px-6 lg:px-8">
      <div className="flex flex-wrap justify-between items-center mb-6 gap-4">
          <h1 className="text-3xl font-bold">Home</h1> {/* Changed title */}
