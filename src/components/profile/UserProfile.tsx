...
import {
  ...
  Star,
  Gift,
} from 'lucide-react'; // Added gamification icons
...
import {
  getUserGamification,
  updateCarpoolEligibility,
  UserGamification,
  UserBadge,
} from '@/services/user-service'; // Import gamification services and types
...
const fetchParkingHistory = async (userId: string): Promise<ParkingHistoryEntry[]> => {
  ...
  return [
    ...
    },
    {
      ...
      locationName: 'Downtown Garage',
      locationId: 'lot_A',
      startTime: '2024-07-25 10:00',
      endTime: '2024-07-25 11:30',
      cost: 3.50,
      status: 'Completed'
    },
    {
      id: 'hist2',
      spotId: 'lot_B-S22',
      locationName: 'Airport Lot B',
      locationId: 'lot_B',
      startTime: '2024-07-23 14:15',
      endTime: '2024-07-23 16:00',
      cost: 5.00,
      status: 'Completed'
    },
    {
...
const getIconFromName = (iconName: string | undefined): React.ElementType => {
  switch (iconName) {
    ...
    case 'Star':
      return Star;
    case 'Megaphone':
      return AlertTriangle; // Reusing for reporter badge
    case 'CheckCircle':
      return CheckCircle;
    case 'Gift':
      return Gift;
...
export default function UserProfile({
  ...
  const {updateUserProfile} = useContext(AppStateContext)!;
  ...
  const handleSaveProfile = () => {
    ...
    // In a real app, you'd send this data to your backend to update the user profile
    console.log("Saving profile changes:", {name: newName, avatarUrl: newAvatarUrl});
    ...
    setUserDetails({...userDetails!, name: newName, avatarUrl: newAvatarUrl});
                ...
                <>
                    ...
                    <div className="space-y-1 flex-grow">
                      <div className="flex justify-between items-center">
                        ...
                        <Badge variant={isPremium ? "default" : "secondary"}
                               className={isPremium ? "bg-yellow-500 text-black" : ""}>
                  ...
                  {!editMode ? (
                    ...
                    <div className="flex gap-2">
                      <Button variant="secondary" size="sm" onClick={handleSaveProfile}>Save</Button>
                  ...
                  <div className="mb-6 space-y-3">
                    <h3 className="text-md font-semibold flex items-center gap-2"><Trophy className="h-4 w-4 text-yellow-600"/>
                      Rewards & Badges</h3>
                  ...
                  <div className="mb-6 space-y-4">
                    <div className="flex justify-between items-center">
                      <h3 className="text-md font-semibold flex items-center gap-2"><DollarSign className="h-4 w-4"/>
                        Billing</h3>
                      ...
                      <CardContent>
                        {isPremium ? (
                          <div className="space-y-1 text-xs">
                            <p className="flex items-center gap-1"><CheckCircle className="h-3 w-3 text-green-600"/> Lower transaction
                              fees</p>
                            <p className="flex items-center gap-1"><CheckCircle className="h-3 w-3 text-green-600"/> {billingInfo?.guaranteedSpotsAvailable ?? 0}
                              Guaranteed Spot passes/month</p>
                        ...
                        {billingInfo?.paymentMethods && billingInfo.paymentMethods.length > 0 ? (
                          billingInfo.paymentMethods.map((method, index) => (
                            <div key={index} className="flex items-center justify-between p-2 border rounded-md text-sm">
                              <div className="flex items-center gap-2">
                                {method.type === 'Card' ? <CreditCard className="h-4 w-4 text-muted-foreground"/> :
